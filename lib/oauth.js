...
module.exports = function(router, passport, user, config) {
  ...
  function registerProvider(provider, configFunction) {
    ...
    if (config.getItem(configRef + '.credentials')) {
      ...
      if(!config.getItem('security.disableLinkAccounts')) {
        router.get(`/${provider}/link/`, passport.authenticate('bearer', {session: false}), passportCallback(provider, options, 'link'));
        router.get(`/${provider}/link/callback`, passport.authenticate('bearer', {session: false}),
  ...
  function registerTokenProvider (providerName, Strategy) {
    ...
    if (config.getItem(configRef + '.credentials')) {
      ...
      if(!config.getItem('security.disableLinkAccounts')) {
        router.post(`/${providerName}/link/token`, passport.authenticate('bearer', {session: false}),
  ...
  function getLinkCallbackURLs(provider, req, operation, accessToken) {
    ...
    if(operation === 'link') {
      var reqUrl = protocol + req.get('host') + req.baseUrl + '/' + provider + '/link/callback';
      // if(accessToken && (stateRequired.indexOf(provider) > -1 || config.getItem('providers.' + provider + '.stateRequired') === true)) {
      //   reqUrl = protocol + req.get('host') + req.baseUrl + '/' + provider + '/link/callback';
      // } else {
      //   reqUrl = protocol + req.get('host') + req.baseUrl + '/' + provider + '/link/callback?state=' + accessToken;
      // }
      return reqUrl;
    }
  }
