...
module.exports = function(config, router, passport, user) {
  ...
  router.post('/logout-all',
    function (req, res, next) {
      ...
      user.logoutUser(null, sessionToken)
        .then(function () {
          res.status(200).json({success: 'Выполнен выход'});
        }, function (err) {
      ...
      user.create(req.body, req)
        .then(function (newUser) {
          if(config.getItem('security.loginOnRegistration')) {
            return user.createSession(newUser._id, 'local', req.get('X-Real-IP') || req.ip)
              .then(function (mySession) {
                res.status(200).json(mySession);
              }, function (err) {
                return next(err);
            ...
            res.status(201).json({success: 'Создан пользователь'});
          }
        }, function (err) {
          return next(err);
        });
  ...
  router.post('/forgot-password', function (req, res, next) {
      user.forgotPassword(req.body.email, req).then(function () {
        res.status(200).json({success: 'Отправлено письмо со ссылкой сброса пароля'});
      }, function (err) {
        return next(err);
      });
    });
      ...
      user.resetPassword(req.body, req)
        .then(function (user) {
          if(config.getItem('security.loginOnPasswordReset')) {
            return user.createSession(user._id, 'local', req.get('X-Real-IP') || req.ip)
              .then(function (mySession) {
                res.status(200).json(mySession);
              }, function (err) {
                return next(err);
            ...
            res.status(200).json({success: 'Успешный сброс пароля'});
          }
        }, function (err) {
          return next(err);
        });
  ...
  router.post('/password-change',
    ...
    function (req, res, next) {
      user.changePasswordSecure(req.user._id, req.body, req)
        .then(function () {
          res.status(200).json({success: 'Пароль изменён'});
        }, function (err) {
          return next(err);
        });
    });
  ...
  router.get('/confirm-email/:token', function (req, res, next) {
    ...
    if (!req.params.token) {
      var err = {error: 'Требуется token проверки email'};
      if(redirectURL) {
    ...
    user.verifyEmail(req.params.token, req).then(function () {
      ...
      res.status(200).send({ok: true, success: 'Email пожтверждён'});
    }, function (err) {
      if(redirectURL) {
  ...
  router.get('/validate-username/:username',
    function(req, res, next) {
      if(!req.params.username) {
        return next({error: 'Не указано имя пользователя', status: 400});
      }
  ...
  router.get('/validate-email/:email',
    function(req, res, next) {
      var promise;
      if(!req.params.email) {
        return next({error: 'Не указан email', status: 400});
      }
  ...
  router.post('/change-email',
    ...
    function (req, res, next) {
      user.changeEmail(req.user._id, req.body.newEmail, req)
        .then(function () {
          res.status(200).json({ok: true, success: 'Email изменён'});
        }, function (err) {
          return next(err);
        });
    });
