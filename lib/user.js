...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validateUsername = function (username) {
    ...
    if (!username.match(USER_REGEXP)) {
      return BPromise.resolve('Ошибка в имени пользователя. Разрешены строчные буквы латинскгого алфавита и цифры, длина от 3 до 16 символов');
    }
    return userDB.query('auth/username', {key: username})
      .then(function (result) {
        if (result.rows.length === 0) {
          ...
          return BPromise.resolve(`Имя '${username}' занято`);
        }
      }, function (err) {
        throw new Error(err);
      });
  ...
  this.validateName = function (username) {
    ...
    if(username.match(/(https?|mailto):/)) {
      return BPromise.resolve('Запрещены ссылки в имени пользователя');
    }
    // Pass!
    return BPromise.resolve();
  };
  ...
  this.validateEmail = function (email) {
    ...
    if (!email.match(EMAIL_REGEXP)) {
      return BPromise.resolve('Неверный формат email');
    }
    return userDB.query('auth/email', {key: email})
      .then(function (result) {
        if (result.rows.length === 0) {
          ...
          return BPromise.resolve(`Email '${email}' занят`);
        }
      }, function (err) {
        throw new Error(err);
      });
      ...
      .then(function(results) {
        if (results.rows.length > 0) {
          ...
          return BPromise.reject({
            error: `newAccount`,
            message: `newAccount:${baseUsername}`,
            status: 409
          });
  ...
  this.addUserDB = function(user_id, dbName, type, designDocs, permissions) {
    ...
    return userDB.get(user_id)
      ...
      .then(function(finalDBName) {
        ...
        if(type === 'shared' && designDocs){
          if(!userDoc.profile.myDBs) {
            userDoc.profile.myDBs = [];
          }
          userDoc.profile.myDBs.push(dbName);
        }
  ...
  this.uniqueDbName = function (name) {
    return dbAuth.allDBs()
      .then((res) => {
        let finalName = name;
        let ind = 0;
        while (res.includes(finalName)) {
          ind++;
          finalName = name + ind;
        }
        return finalName;
