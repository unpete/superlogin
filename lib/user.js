      ...
      .then(function(results) {
        if (results.rows.length > 0) {
          ...
          return BPromise.reject({
            error: `Пользователь '${baseUsername}' провайдера '${provider}' не связан с пользователем сервиса`,
            message: 'Для авторизации oAuth, зарегистрируйтесь через логин/пароль и свяжите учётную запись сервиса с провайдером социальной сети',
            status: 409
          });
